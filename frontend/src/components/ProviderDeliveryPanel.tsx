...
export const ProviderDeliveryPanel: React.FC<ProviderDeliveryPanelProps> = ({ controller, gitOperations, changeSets, readOnly, onFollowUpCreated, onOpenFollowUpRun, onShowOfflineBundles }) => {
  ...
  return <div className="provider-delivery-panel">
    ...
    <div className="delivery-form provider-publish-form">
      <div className="delivery-field"><span>{t("delivery.provider")}</span><WorkbenchSelect label={t("delivery.provider")} value={providerConfig?.id || ""} onChange={(value) => { setProviderConfigId(value); controller.clearError(); }} options={[{ value: "", label: t("delivery.selectProvider") }, ...controller.providers.map((item) => ({ value: item.id, label: item.id, meta: item.kind }))]} /></div>
      ...
      <div className="delivery-field"><span>{t("delivery.verifiedCommit")}</span><WorkbenchSelect label={t("delivery.verifiedCommit")} value={selectedOperation?.id || ""} onChange={setOperationId} options={[{ value: "", label: t("delivery.selectVerifiedCommit") }, ...completedCommits.map((item) => ({ value: item.id, label: String(item.after?.headSha || item.preflight.evidenceSummary?.headSha).slice(0, 12), meta: item.preflight.evidenceSummary?.changeSetId.slice(0, 12) }))]} /></div>
