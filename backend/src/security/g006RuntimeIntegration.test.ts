...
test("signed registry hooks gate loop, websocket, and task completion, fail open with trace evidence, and reload without duplication", async (t) => {
  ...
  const ws = new FakeSocket(); handleChatWs(ws as unknown as WebSocket, sessionFor(workspace), { validateSession: () => true }); await withModelFetch(async () => { ws.emit("message", Buffer.from(JSON.stringify({ type: "message", requestId: "closed-ws", message: "finish", mode: "ask", modelName: config.modelName }))); await waitFor(() => ws.frames.some((frame) => frame.type === "run_state" && frame.status === "failed"), 5_000, () => ws.frames); }, origin);
