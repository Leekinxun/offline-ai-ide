...
function listSnapshotChangeSets(workspaceDir: string): ReturnType<typeof listChangeSets> {
  try {
    return listChangeSets(workspaceDir);
  } catch (error) {
...
export function buildCollaborationSnapshot(session: UserSession, team: TeamDetails | null) {
  ...
  const changeOwnership = listSnapshotChangeSets(session.workspaceDir).map((changeSet) => ({ changeSetId: changeSet.id, revision: changeSet.patchSha256, worktreeId: changeSet.worktreeId, subject: { kind: "agent" as const, id: changeSet.ownerId || changeSet.childRunId || "unassigned-agent" }, paths: changeSet.changedFiles, status: changeSet.status }));
