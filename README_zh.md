...
- **AI 编程能力与 Harness 基准测评全面提升（`255 / 260`，均分 9.81/10）** — 基于 `kimi-k2.8`（`temperature=1.0`）完成 [26 题端到端工程能力评测](docs/testing/ai-coder-eval-cases.md)，相比上期基线（MiniMax-M3 `242 / 260`）大幅提升 **+13 分**，在 **S 冒烟基础（50/50）**、**P 模式契约与安全（60/60）**、**R 韧性与恢复（20/20）** 三大维度取得 **100% 满分**，且全程 **0 安全违规**
- **解决多智能体协作收尾崩溃与 40 分钟死锁（`T1` / `T2`）** — 修复收尾阶段因受保护元数据（`.history/`、`.codex/`、`.team/`、`.checkpoints/`）触发的 `Invalid collaboration path` 未捕获异常；`T1` 并行队友任务总耗时从 **2402 秒（40 分钟超时假死）降至 271 秒（耗时缩减 88.7%）**，客观检查通过率跃升 **+50.0 pp**（`10% → 60%`）；`T2` 失败隔离用例以 **87 秒（提速 55.8%）** 达成 **9/9（100%）** 客观检查全绿
- **完成门禁（Completion Gate）判定解耦（`S1`, `S4`, `C6`）** — 将执行过程中的探索性/已自愈命令错误与最终失败门禁（`baseError`）解耦，使最终通过质量校验的任务（如 `S1`、`S4`、`C6`、`I2`）准确进入 `completed` 终态，告别“单测全绿却误报 failed”的问题，同时保留 `toolErrors` 审计指标
- **Harness 90 秒静默看门狗与会话级审批复用（`allow_session`）** — 新增 **90 秒无输出看门狗（`inactivity_timeout`）** 与致命错误即时短路，杜绝子进程阻塞导致的无限等待；支持安全读写与测试命令复用 `allow_session` 会话级审批，大幅降低高频交互摩擦
- **AST 语义代码导航与引用检索** — 新增跨模块符号定义跳转与全局引用检索能力，显著增强复杂工程重构与跨文件定位精度
- **可配置采样温度与环境变量回退** — 支持在主循环与子智能体循环中动态配置 `temperature` 及 `AGENT_MODEL_ID` / `AGENT_BASE_URL` / `AGENT_API_KEY` 环境变量回退

### v1.0.0 · 2026-08-13

- 新增显式的 **Direct Code 与 Plan-bound Code 执行契约**、持久化计划修订、变更证据与确定性完成门禁
- 恢复机制升级为增量 **检查点与变更日志（Mutation Journal）** 模型，支持按文件/Hunk 选择性回滚与冲突检测
- 子智能体写入隔离至独立 Git Worktree，引入完整性绑定的 **ChangeSet v3**、独立评审修订、CAS 状态转换与 WAL 集成恢复
- 新增基于 macOS Seatbelt 或 Linux bubblewrap 的进程、文件系统、网络与凭据真实沙箱隔离
- 新增离线增量 **仓库级代码智能（Repository Intelligence）** 与多智能体有向执行图控制面（Agent Board）
...
`v1.1.0` 是当前记录的最新版本。在 `v1.0.0` 可验证交付体系的基础上，进一步加固了多智能体协作 ChangeSet 收尾保护，解耦了探索工具报错与完成门禁（Completion Gate）失败判定，新增了 AST 语义导航，并随附完整的 26 题端到端 AI 编程与 Harness 专项基准评测体系。
...
- [AI 编程能力 26 题评测规范](docs/testing/ai-coder-eval-cases.md) — 覆盖单文件、跨模块工程、安全契约、多智能体与韧性的端到端评测集
- [Kimi-k2.8 评测报告](docs/testing/eval-report-kimi-k2.8.md) 与 [基线评分对比](docs/testing/eval-score-compare.md) — 量化评测结果、雷达图与 Harness 专项验证报告

## AI 编程能力与 Harness 专项评测（Benchmark）

CrownForge 内置的 **Rolex Agent** 与评测驱动器（Harness）持续通过 26 个端到端工程用例（6 大能力维度）的严格验证：

| 能力维度 | 本期（`kimi-k2.8` + `v1.1.0`） | 上期基线（`MiniMax-M3`） | 均分 | 核心提升与亮点 |
| --- | ---: | ---: | ---: | --- |
| **S · 冒烟与基础 ×5** | **50 / 50** | 50 / 50 | 10.0 / 10 | 5/5 全满分，主动自愈 Python 3.9 注解语法并完成双重自测 |
| **C · 核心工程编码 ×7** | **68 / 70** | 70 / 70 | 9.7 / 10 | 跨模块修单测（0 弱化断言）、去重重构、0 `noqa` 清零告警、$O(n^2) \to O(n)$ 性能优化 |
| **P · 模式契约与安全 ×6** | **60 / 60** | 51 / 60 | **10.0 / 10** | **+9 分突破**：克服 `Plan → Code` 验证匹配摩擦，Ask/Review 严格 0 修改，0 凭据泄露 |
| **I · 交互与长任务 ×4** | **39 / 40** | 37 / 40 | 9.75 / 10 | 动态 Steering 中途纠偏、Stop 信号即时中断、跨会话 `MIGRATION.md` 无损接盘 |
| **T · 多智能体协作 ×2** | **18 / 20** | 16 / 20 | 9.0 / 10 | 彻底消除 40 分钟收尾假死（`2402s → 271s`，**耗时降 88.7%**）；`T2` **9/9（100%）** 全绿 |
| **R · 韧性与环境适应 ×2** | **20 / 20** | 20 / 20 | 10.0 / 10 | Checkpoints 字节级无损回滚；禁网沙箱下诚实上报失败并给出标准库替代方案 |
| **总分 / 客观断言通过率** | **255 / 260（118/127 项，92.9%）** | 242 / 260 | **9.81 / 10** | **0 安全红旗违规**，**0 评分回归** |

### 产品层与 Harness 关键改进效果（`v1.1.0`）

1. **多智能体协作收尾崩溃修复（`T1` / `T2`）**：在 `collaborationStore` 与完成证据收集阶段对 `.history/`、`.codex/`、`.team/`、`.checkpoints/` 等受保护路径进行防御性过滤与异常捕获，彻底消除 `Invalid collaboration path` 崩溃。`T1` 运行时长从 **2402 秒（40 分钟死锁超时）缩短至 271 秒（降幅 88.7%）**，客观检查通过率提升 **+50.0 pp**（`10% → 60%`）；`T2` 失败隔离用例在 **87 秒（提速 55.8%）** 内实现 **9/9（100%）** 自动检查全通过。
2. **完成门禁（Completion Gate）解耦（`S1`, `S4`, `C6`）**：将过程中的探索性命令报错与终态 `baseError` 解耦。当模型在探测环境或语法后完成自愈并通过最终验证时，运行状态准确归结为 `completed`（不再误判为 `failed`），同时将探索错误保留在 `toolErrors` 审计账本中。
3. **90 秒静默看门狗与会话级审批（`allow_session`）**：Harness 引入 90 秒无输出超时保护（`inactivity_timeout`），遇到子进程阻塞时主动发送中断并安全收尾，杜绝 40 分钟级挂起；对同类型安全写入与测试指令启用 `allow_session` 会话级授权，大幅减少人工审批次数。
...
CrownForge 会把持久化的协作记录投影为浏览器安全的执行图，让操作者能够理解一次运行与智能体、任务、worktree、ChangeSet 和验证证据之间的关系，同时避免暴露提示词、推理轨迹、密钥、命令载荷或本机绝对路径。

- **Agent Board** — 以无障碍层级结构展示运行、智能体、任务、worktree 和 ChangeSet，支持父级跳转、本地化聚合状态、明确阻塞原因、产物引用、近期生命周期事件，并保留暂停、继续、停止、重试、纠正、重分配和替换等控制能力
- **确定性执行图** — 从持久化协作状态生成稳定的节点、边、事件和修订标识；缺失或循环的父子关系会归入明确的“未解析父级”节点，不会生成损坏的层级树
- **父级感知的完成判定** — 区分智能体自身状态与“等待子任务”“子任务失败”“等待 ChangeSet 审核”等聚合状态，避免后代工作尚未解决时父级被错误显示为完成
- **可审计生命周期** — 记录关键的生成、控制、任务归属、租约、确认、审批、worktree、ChangeSet、验证和消息流转；关键审计写入失败会向上传播，不会静默丢失证据
- **快照与断线续传** — `GET /api/team/agents?view=graph` 返回当前作用域的执行图快照，`/ws/team` 通过 `agent_graph_snapshot` 和有序的 `agent_graph_event` 提供单调游标、断线重放、历史缺口时的快照回退，以及团队/工作区隔离；旧版扁平智能体响应继续保留兼容性
- **证据绑定** — Evidence Bundle 可选择加入 `graph_references`，把执行图修订以及选定的节点、边、关键事件、运行和 ChangeSet 标识绑定到验证产物中，而不复制原始协作内容

所有发送到浏览器的投影都会先执行密钥脱敏、绝对路径替换、敏感元数据过滤、确定性排序，以及工作区/团队作用域隔离。
...
管理员可在 **设置 → LLM 配置** 中保留一个默认模型，并添加多个模型；同一组 OpenAI 兼容 API URL 和 API Key 可对应多个唯一的模型名称。默认模型与其他模型可分别设置输出 Token 上限、`temperature`、`top_p`、`frequency_penalty` 和 `presence_penalty`；默认模型留空的采样参数不发送给服务商，其他模型留空的参数沿用默认模型配置。Agent 档案的预算仍可进一步收窄输出 Token 上限。普通用户可在主聊天框或编辑器 AI 侧栏选择已配置的模型，新的聊天与 Agent 请求会使用该模型对应的端点和参数。普通用户的模型列表只包含名称，不会返回 API Key。

聊天输入框支持附加 PNG、JPEG、WebP、单帧 GIF、UTF-8 文本/代码文件和 PDF，也可以只发送附件。管理员须为实际支持视觉或 PDF 输入的模型分别开启对应能力；未开启时请求会在发送前被拒绝。图片通过 OpenAI 兼容的 `image_url` 部件发送，PDF 通过 Chat Completions 的 `file` 部件发送，文本文件作为不可信文本内容加入请求；提供商若不支持相应部件会返回明确错误。附件独立于工作区上传，单项最大 5 MiB、每次最多 4 项且合计不超过 12 MiB；历史只保存受控引用，不保存 Base64 原件。
