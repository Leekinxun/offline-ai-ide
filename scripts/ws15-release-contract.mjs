...
for (const token of ["crownforge-permissions:", "command: [\"init-mounts\"]", "cap_add:", "- CHOWN", "depends_on:", "- crownforge-permissions", "user: \"${CROWNFORGE_UID:-10001}:${CROWNFORGE_GID:-10001}\"", "CROWNFORGE_UID=${CROWNFORGE_UID:-10001}", "CROWNFORGE_GID=${CROWNFORGE_GID:-10001}", "TEAM_STORE_ROOT=/app/config", "NPM_REGISTRY: \"${NPM_REGISTRY:-https://registry.npmmirror.com}\""]) if (!composeSource.includes(token)) failures.push(`docker-compose.yml is missing automatic mount initialization token ${JSON.stringify(token)}`);
