...
- Debug session runtimes: 调试工作台同时支持 Node.js JavaScript 与 Python 文件。JavaScript 继续复用 Node Inspector；Python 通过标准库 `pdb` 提供启动、断点、暂停、继续、单步进入/越过/跳出、停止和调用栈。两种运行时共享工作区相对路径、团队只读权限、输出上限和进程组终止边界，但不宣称完整 DAP、变量检查器、表达式求值或 Rust 调试兼容。
- Python debug interaction: 打开 `.py` 或 `.pyw` 文件时自动填入调试目标并允许在当前行切换断点；调试状态必须明确标识 Python 运行时。脚本正常结束后会话进入 stopped，不沿用 `pdb` 默认的自动重启；解释器缺失、目标越界和不支持的扩展名必须显示可操作错误，不能表现为无反馈按钮。
